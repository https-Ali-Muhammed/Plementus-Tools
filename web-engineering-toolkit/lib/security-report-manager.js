...
function buildHtml(summary) {
  ...
  const frameworkCards = (summary.frameworkResults || []).map((framework) => `
    <div class="framework-card">
      <div><span>${escapeHtml(framework.label)}</span><strong>${framework.technicalCoverage == null ? '—' : `${framework.technicalCoverage}%`}</strong></div>
      <p>${framework.passed} passed · ${framework.attention} attention · ${framework.manualReview} manual/info</p>
    </div>`).join('');
    ...
    <details class="group">
      ...
      <div class="items">${checks.sort((a,b) => statusRank(b.status)-statusRank(a.status)).map((check) => `
        <article class="check">
          ...
          <div><h3>${escapeHtml(check.title)}</h3><p>${escapeHtml(check.summary)}</p>${check.details ? `<small>${escapeHtml(check.details)}</small>` : ''}${check.recommendation ? `<div class="recommend"><b>Recommendation</b>${escapeHtml(check.recommendation)}</div>` : ''}</div>
        </article>`).join('')}</div>
    </details>`).join('');
  ...
  const crawlSection = crawlPages.length ? `
    ...
    <div class="items" style="border:1px solid var(--border);border-radius:16px;background:var(--panel);padding:14px;display:grid;gap:8px">
      ${crawlPages.map((p) => `<div style="display:flex;justify-content:space-between;gap:12px;font-size:12px;color:${p.found ? '#c9d2e5' : '#6c7690'}"><span>${escapeHtml(p.url)}</span><span>${p.found ? `${p.status} · ${escapeHtml((p.groups||[]).join(', '))}` : 'not found'}</span></div>`).join('')}
    </div>` : '';
...
async function writeXlsx(root, summary) {
  ...
  overview.getCell('D9').value='Framework technical coverage';overview.getCell('D9').font={bold:true,color:{argb:muted}};
  summary.frameworkResults.forEach((fw,i)=>{const r=10+i;overview.getCell(r,4).value=fw.label;overview.getCell(r,4).font={bold:true,color:{argb:text}};overview.getCell(r,5).value=fw.technicalCoverage==null?'—':`${fw.technicalCoverage}%`;overview.getCell(r,6).value=`${fw.passed} passed`;overview.getCell(r,7).value=`${fw.attention} attention · ${fw.manualReview} manual/info`;});

  const checks = workbook.addWorksheet('Security Checks', { views: [{ state:'frozen', ySplit:1, showGridLines:false }] });
  checks.columns=[{header:'Category',width:24},{header:'Check',width:34},{header:'Status',width:18},{header:'Summary',width:58},{header:'Details / Evidence',width:65},{header:'Recommendation',width:65},{header:'Frameworks',width:42}];
  ...
  for(const item of summary.checks){const row=checks.addRow([item.category,item.title,statusLabel(item.status),item.summary,[item.details,item.evidence].filter(Boolean).join(' · '),item.recommendation,(item.frameworks||[]).join(', ')]);row.height=44;row.alignment={vertical:'top',wrapText:true};row.getCell(3).fill={type:'pattern',pattern:'solid',fgColor:{argb:statusFill(item.status)}};row.getCell(3).font={bold:true,color:{argb:statusColor(item.status)}};row.eachCell(c=>c.border={bottom:{style:'thin',color:{argb:border}}});}
  checks.autoFilter={from:'A1',to:'G1'};

  const mapping = workbook.addWorksheet('Compliance Mapping', { views: [{ state:'frozen', ySplit:1, showGridLines:false }] });
  mapping.columns=[{header:'Framework',width:24},{header:'Technical Coverage',width:20},{header:'Passed',width:14},{header:'Attention',width:14},{header:'Manual / Info',width:16},{header:'Jurisdiction',width:24},{header:'Scope Note',width:72}];
  ...
  for(const fw of summary.frameworkResults){const row=mapping.addRow([fw.label,fw.technicalCoverage==null?'—':`${fw.technicalCoverage}%`,fw.passed,fw.attention,fw.manualReview,fw.jurisdiction||'',fw.note]);row.height=42;row.alignment={vertical:'top',wrapText:true};row.eachCell(c=>c.border={bottom:{style:'thin',color:{argb:border}}});}
  ...
  if (summary.crawl && Array.isArray(summary.crawl.pages) && summary.crawl.pages.length) {
    ...
    evidence.columns=[{header:'URL',width:60},{header:'Found',width:12},{header:'HTTP Status',width:14},{header:'Evidence Group(s)',width:32}];
    ...
    for(const page of summary.crawl.pages){const row=evidence.addRow([page.url,page.found?'Yes':'No',page.status||'','' + (page.groups||[]).join(', ')]);row.height=24;row.alignment={vertical:'top',wrapText:true};row.eachCell(c=>c.border={bottom:{style:'thin',color:{argb:border}}});}
    evidence.autoFilter={from:'A1',to:'D1'};
  }
...
export class SecurityReportManager {
  ...
  async save(summary) {
    ...
    const rows = [['Category','Check','Status','Summary','Details / Evidence','Recommendation','Frameworks']];
    for (const item of summary.checks) rows.push([item.category,item.title,statusLabel(item.status),item.summary,[item.details,item.evidence].filter(Boolean).join(' · '),item.recommendation,(item.frameworks||[]).join(', ')]);
