...
export function buildBrokenLinksSummaryHtml(result) {
  ...
  </main>${reportBackToTopControl()}</body></html>`;
  return html;
}

function writeHtml(root, result) {
  fs.writeFileSync(path.join(root, 'summary.html'), buildBrokenLinksSummaryHtml(result));
}
...
export class BrokenLinksReportManager {
  ...
  async save(result) {
    ...
    return {
      ...
      jsonHref: reportDownloadHref(reportName, 'json'),
