...
export function reportHtmlTheme({ accent = '#70dfd0', accentSoft = 'rgba(112,223,208,.12)' } = {}) {
  return `
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-width:320px;background:radial-gradient(circle at 82% 0,rgba(62,100,163,.16),transparent 30%),linear-gradient(180deg,#0b1221,#080d18);color:var(--report-text);font:14px/1.55 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input,select{font:inherit}button{color:inherit}
    ...
    ${reportBackToTopStyles({ accent, panel: 'rgba(17,27,47,.96)' })}
    @media(max-width:900px){.report-meta{grid-template-columns:repeat(2,minmax(0,1fr))}.report-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.report-shell{padding:24px 14px 58px}.report-header,.report-section{padding:16px}.report-meta,.report-metrics{grid-template-columns:1fr}.report-section-heading{display:grid;gap:8px}.report-table{min-width:620px}}@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}@media print{.screen-only,.back-to-top{display:none!important}.report-shell{max-width:none;padding:0}.report-header,.report-section{break-inside:avoid;box-shadow:none}}.report-table tr strong{ margin-right:1rem}
  `.trim();
}
...
export function reportHtmlActionStyles() {
  return `.report-actions,.report-action-controls{display:flex;flex-wrap:wrap;gap:8px}.report-action{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:8px 13px;border:1px solid rgba(139,124,255,.48);border-radius:10px;background:linear-gradient(180deg,rgba(124,108,255,.18),rgba(124,108,255,.1));color:#f7f9ff;font:750 12px/1.2 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;text-decoration:none;white-space:nowrap;cursor:pointer;transition:background .16s ease,border-color .16s ease,box-shadow .16s ease,transform .16s ease}.report-action:hover{border-color:rgba(169,159,255,.78);background:linear-gradient(180deg,rgba(124,108,255,.3),rgba(124,108,255,.18));color:#fff;transform:translateY(-1px)}.report-action:focus-visible{outline:3px solid #a99fff;outline-offset:3px}.report-action:active{transform:translateY(0)}.report-action[aria-disabled="true"],.report-action:disabled{opacity:.5;cursor:not-allowed;pointer-events:none;transform:none}@media(max-width:560px){.report-actions,.report-action-controls{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.report-action{width:100%}}@media(max-width:380px){.report-actions,.report-action-controls{grid-template-columns:1fr}}@media(prefers-reduced-motion:reduce){.report-action{transition:none}}`;
}
...
export function reportBackToTopControl() {
  return `<button class="back-to-top screen-only" type="button" aria-label="Back to top" data-report-back-to-top hidden>↑</button><script>(()=>{const button=document.querySelector('[data-report-back-to-top]');if(!button)return;const reduced=matchMedia('(prefers-reduced-motion: reduce)');const update=()=>{button.hidden=window.scrollY<400};button.addEventListener('click',()=>window.scrollTo({top:0,behavior:reduced.matches?'auto':'smooth'}));window.addEventListener('scroll',update,{passive:true});update()})();</script>`;
}

export function reportBackToTopStyles({ accent = '#70dfd0', panel = '#111b2f' } = {}) {
  return `.back-to-top{position:fixed;z-index:90;right:max(16px,calc(env(safe-area-inset-right) + 12px));bottom:max(16px,calc(env(safe-area-inset-bottom) + 12px));display:grid;place-items:center;width:42px;height:42px;padding:0;border:1px solid color-mix(in srgb,${accent} 42%,transparent);border-radius:999px;background:${panel};box-shadow:0 10px 26px rgba(0,0,0,.32);color:${accent};font:850 20px/1 Inter,ui-sans-serif,system-ui,sans-serif;cursor:pointer;transition:transform .16s ease,background .16s ease}.back-to-top:hover{transform:translateY(-2px)}.back-to-top:focus-visible{outline:3px solid ${accent};outline-offset:3px}.back-to-top[hidden]{display:none}@media(prefers-reduced-motion:reduce){.back-to-top{transition:none}}@media(max-width:560px){.back-to-top{right:max(12px,calc(env(safe-area-inset-right) + 10px));bottom:max(12px,calc(env(safe-area-inset-bottom) + 10px))}}@media print{.back-to-top{display:none!important}}`;
}
