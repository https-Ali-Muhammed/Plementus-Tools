...
import { generateToolPdf } from '../lib/pdf-report-renderer.js';
...
test('canonical report filenames are descriptive, safe, and share the report timestamp', () => {
  ...
  assert.throws(() => buildReportDownloadFilename({ reportType: 'lighthouse', extension: 'xlsx' }), /Unsupported/);
});
...
test('new native PDFs are A4, searchable, bounded, and Unicode-safe', { timeout: 30_000 }, async (t) => {
  ...
  const generatedAt = '2026-08-30T13:55:42.123Z';
  const longUrl = `https://example.test/${'long-segment-'.repeat(24)}?token=%5BREDACTED%5D`;
  const documents = [
  ...
  for (const [name, title, html] of documents) {
    ...
    await generateToolPdf({ html, pdfPath, toolName: title, projectName: 'مشروع Fixture', browserPath: capability.browser.path });
    assert.equal(fs.readFileSync(pdfPath).subarray(0, 5).toString(), '%PDF-');
    const info = spawnSync('pdfinfo', [pdfPath], { encoding: 'utf8' });
    if (!info.error && info.status === 0) assert.match(info.stdout, /Page size:\s+59[45]\.\d+ x 84[12]\.\d+ pts \(A4\)/);
...
test('real report download endpoint returns canonical historical PDF and CSV filenames for every tool', { timeout: 15_000 }, async (t) => {
  ...
  const expected = {
    lh: ['lighthouse-report__lighthouse-project__2026-08-30_14-55-02Z.pdf', 'lighthouse-report__lighthouse-project__2026-08-30_14-55-02Z.csv'],
    asset: ['asset-page-weight-report__asset-project__2026-08-30_14-56-03Z.pdf', 'asset-page-weight-report__asset-project__2026-08-30_14-56-03Z.csv'],
    links: ['broken-links-resources-report__links-project__2026-08-30_14-57-04Z.pdf', 'broken-links-resources-report__links-project__2026-08-30_14-57-04Z.csv'],
    comp: ['compliance-mapping-report__compliance-project__2026-08-30_14-58-05Z.pdf', 'compliance-mapping-report__compliance-project__2026-08-30_14-58-05Z.csv']
  };
  for (const [name] of fixtures) {
    for (const [index, format] of ['pdf', 'csv'].entries()) {
      ...
      assert.match(response.headers.get('content-type') || '', format === 'pdf' ? /^application\/pdf/ : /^text\/csv/);
      await response.arrayBuffer();
    }
  }
  const blocked = await fetch(`http://127.0.0.1:${port}/api/reports/lh/download/xlsx`);
...
test('tool PDF documents share the report family while retaining truthful sections', () => {
  ...
  for (const html of [lighthouse, asset, links]) assert.match(html, /Web Engineering Toolkit/);
